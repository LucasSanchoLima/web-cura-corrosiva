...
import { requesteAlterarTitulo } from "@/components/funcoes/funcoes";
...
export default function AlterarNomeLivro() {
  ...
  async function criarNome(formData: FormData) {
    const resultado = await requesteAlterarTitulo(await user!.getIdToken(), String(formData.get("nome")), variaveis[0]);
    ...
    redirect("/autor/" + variaveis[1] + "/" + String(formData.get("nome")).replaceAll(" ", "_"));
    mudaEstadoPopUp(0);
  }
