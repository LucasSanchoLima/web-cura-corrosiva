...
import { adicionarCapitulo, requesteAlterarTitulo } from "@/components/funcoes/funcoes";
import { usePopUpContext } from "@/contexts/popUpContext";
import { useState } from "react";
import { redirect } from "next/navigation";

export default function AlterarNomeLivro() {
  const { textoBotao, mudaEstadoPopUp, variaveis } = usePopUpContext();
  const { user } = useUserContext();
  const [erro, setErro] = useState("");

  async function criarNome(formData: FormData) {
    const resultado = await adicionarCapitulo(await user!.getIdToken(), String(formData.get("nome")), variaveis[0]);
    ...
    redirect("/autor/" + variaveis[1] + "/" + variaveis[0] + "/" + String(formData.get("nome")).replaceAll(" ", "_"));
  }
  ...
  return (
    <div className=" relative flex justify-center  items-center flex-col">
      ...
      >
        <p className="mb-1 ml-2 text-sm sm:text-base">Titulo</p>
        <input
          type="text"
          name="nome"
